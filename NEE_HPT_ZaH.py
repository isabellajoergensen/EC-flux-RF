...
Uses features selected by RFECV from Stage 1 (NEE_RFE_ZaH.py)
...
output_dir = Path(__file__).resolve().parent / "output" / site_ID / "NEE_HPT_ZaH"
...
param_distributions = {
    'rf__n_estimators': randint(100, 400),           # Number of trees
    'rf__max_depth': [5, 10, 15, 20, 30],     # Maximum tree depth
...
print(f"  n_estimators: 100-400 (uniform)")
print(f"  max_depth: [5, 10, 15, 20, 30]")
...
stage1_metrics_file = Path(__file__).resolve().parent / "output" / site_ID / "NEE_RFE_ZaH" / 'model_performance_metrics.csv'
...
print("="*70)
print("✓ Stage 1 (NEE_RFE_ZaH.py): Feature selection with RFECV")
print("✓ Stage 2 (NEE_HPT_ZaH.py): Hyperparameter tuning with selected features")
print("\nData Leakage Check:")
